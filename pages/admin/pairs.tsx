...
> = async ({ req, res, query }) => {
	...
	const q = {
		$or: [
			...
			{ "profileInfo.gender": { $regex: sq } },
		],
	};

	const result = query.searchQuery
		? await Pair.paginate(q, options)
		: await Pair.paginate({}, options);
							...
							{query.docs.map((usr, idx) => {
								return (
									<Tr key={idx}>
										...
										<Td>{usr.profileInfo.gender}</Td>
										<Td>{usr.profileInfo.school}</Td>
										<Td>
											<Link href={`/admin/pair/${usr._id}`}>
